...
{
    public class ProveedorHistoricoJsonSUBE2 : IQPAProveedorPuntosHistoricos<string>
    {
        ...
        public bool DeleteExtractDirectory { get; set; } = false;

        public Dictionary<string, List<PuntoHistorico>> Get()
        {
            // busco archivos zip fechados...
            var zipsEnFecha = Directory.EnumerateFiles(InputDir, ZipPrefix + ".zip")
                .Where(z => DameFechaArchivoZip(z) >= TruncarFechaYMD(FechaDesde))
                .Where(z => DameFechaArchivoZip(z) < FechaHasta)
                .ToList()
            ;

            // si existen los descomprimo a un subdirectorio...
            string extractDirName = string.Empty;
            foreach (var zipX in zipsEnFecha)
            {
                extractDirName = DescomprimirZipEn(zipX, InputDir, overwrite: true);
            }
            ...
            if (DeleteExtractDirectory)
            {
                Directory.Delete(extractDirName, true);
            }

            return ret;
        }

        string DescomprimirZipEn(string zipPath, string outputDir, bool overwrite)
        {
            ...
            {
                var zipNameOnly = Path.GetFileNameWithoutExtension(zipPath);
                var outputPath  = Path.Combine(outputDir, zipNameOnly, zae.Name);
                newOutputDir = Path.GetDirectoryName(outputPath);
                if (!Directory.Exists(newOutputDir)) { Directory.CreateDirectory(newOutputDir); }
                zae.ExtractToFile(outputPath, overwrite);
            }

            return newOutputDir;
        }

        DateTime TruncarFechaYMD(DateTime fecha)
        {
            return new DateTime(
                year: fecha.Year,
                month: fecha.Month,
        ...
        private static DateTime DameFechaArchivoZip(string filePath)
        {
            if (!File.Exists(filePath))
            {
                throw new ArgumentException($"El archivo {filePath} no existe", nameof(filePath));
